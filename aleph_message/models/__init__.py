...
# Maximum decoded size of a signed IPNS record carried by a STORE message.
MAX_IPNS_RECORD_SIZE = 10240
...
# POST and AGGREGATE messages carry tags in the user-controlled nested
# ``content`` dict (legacy location). When the ``tags`` key is present it
# must match the canonical Tag list shape.
_NESTED_TAGS_ADAPTER: TypeAdapter = TypeAdapter(
    Annotated[List[Tag], Field(max_length=MAX_N_TAGS)]
)


def _validate_nested_tags(value: Any) -> Any:
    if isinstance(value, dict):
        tags = value.get("tags")
        if tags is not None:
            _NESTED_TAGS_ADAPTER.validate_python(tags)
    return value
...
class PostContent(BaseContent):
    ...
    def check_nested_tags(cls, v):
        return _validate_nested_tags(v)
...
class StoreContent(BaseContent):
    ...
    ipns_record: Optional[str] = Field(
        default=None,
        description=(
            "Base64-encoded signed IPNS record. Only allowed when "
            "item_type is 'ipns'; omitted for track-only registrations."
        ),
    )
    max_size_mib: Optional[int] = Field(
        default=None,
        gt=0,
        description="Paid size quota for IPNS registrations, in MiB.",
    )
    tags: Optional[List[Tag]] = Field(default=None, max_length=MAX_N_TAGS)
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Metadata associated with the stored content"
    )
    payment: Optional[Payment] = Field(
        default=None,
        description="Payment information for storage. If not specified, defaults to hold tier.",
    )

    @field_validator("payment")
    def check_payment_type(cls, v: Optional[Payment]) -> Optional[Payment]:
        if v is not None and v.type not in (PaymentType.hold, PaymentType.credit):
            raise ValueError(
                "Only 'hold' and 'credit' payment types are supported for store messages"
            )
        return v

    @model_validator(mode="after")
    def check_ipns_fields(self) -> "StoreContent":
        if self.item_type == ItemType.ipns:
            if self.max_size_mib is None:
                raise ValueError("max_size_mib is required for IPNS store messages")
            if self.ref is not None:
                raise ValueError(
                    "ref is not allowed for IPNS store messages, "
            ...
            if self.ipns_record is not None:
                try:
                    ...
                    raise ValueError("ipns_record must be valid base64") from e
                if len(record) > MAX_IPNS_RECORD_SIZE:
                    raise ValueError(
                        f"ipns_record exceeds maximum size "
                        f"({len(record)} > {MAX_IPNS_RECORD_SIZE} bytes)"
                    )
        else:
            if self.ipns_record is not None or self.max_size_mib is not None:
                raise ValueError(
                    "ipns_record and max_size_mib are only allowed when "
                    "item_type is 'ipns'"
                )
        return self
